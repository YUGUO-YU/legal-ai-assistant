...
async function load() {
  loading.value = true
  try {
    const res = await api.get('/admin/{table}/list'.replace('{table}', 'sys_config'), {
...
async function handleSave() {
  ...
  try {
    let res
    if (form.id) {
      res = await api.post(`/admin/{table}/${form.id}/update`.replace('{table}', 'sys_config'), form)
    } else {
      res = await api.post('/admin/{table}/create'.replace('{table}', 'sys_config'), form)
    }
    if (res.data?.ok) {
...
async function handleDelete(row) {
  try {
    ...
    const res = await api.post(`/admin/{table}/${row.id}/delete`.replace('{table}', 'sys_config'))
    if (res.data?.ok) {
...
.sys-config-page { animation: fadeIn 0.4s ease; padding: 0 4px; }
